...
#    adata = adata_temp[:, gene_list].copy() #subset based on the genes (keep the genes if it was converted to ensemble successfully)
    ...
    prefixData = args.cellex_out_prefix + "." + args.level 
    dirOut = args.outdir
    eso.save_as_csv(file_prefix=prefixData, path=dirOut, verbose=True)

    # replace " " and "/" in the cell type name
    cellex_output = pd.read_csv(os.path.join(args.outdir, args.cellex_out_prefix + "." + args.level + ".esmu.csv.gz"))
    cellex_output.columns = cellex_output.columns.str.replace(" ", "_")
    cellex_output.columns = cellex_output.columns.str.replace("/", "_")
    cellex_output.to_csv(os.path.join(args.outdir, args.cellex_out_prefix + "." + args.level + ".esmu_fmt.tsv"), sep='\t', index=False)
