...
def main(args):
    ...
    """
    outdir = args.outdir
    adata_original = anndata.read_h5ad(args.h5ad,"r")
    level = args.level
    ...
    adata = adata_original[:,gene_list].to_memory()

     # remove cells belonging to small clusters
    cluster_sizes = adata.obs[ct_colname].value_counts()
    min_cluster_size = 20
    small_clusters = cluster_sizes[cluster_sizes < min_cluster_size].index
    cells_to_keep = ~adata.obs[ct_colname].isin(small_clusters)
    ...
    means_cell_log_counts_pM_out.to_csv(os.path.join(outdir,args.id + "." + args.level + "_means_cell_log_counts_pM.tsv"), sep="\t", index=False) # save to a file)
    sds_cell_log_counts_pM_out.to_csv(os.path.join(outdir,args.id + "." + args.level + "_sds_cell_log_counts_pM.tsv"), sep="\t", index=False) # save to a file)
    cov_cell_log_counts_pM_out.to_csv(os.path.join(outdir,args.id + "." + args.level + "_cov_cell_log_counts_pM.tsv"), sep="\t", index=False) # save to a file)
    varom_cell_log_counts_pM_out.to_csv(os.path.join(outdir,args.id + "." + args.level + "_varom_cell_log_counts_pM.tsv"), sep="\t", index=False) # save to a file)
    spec_cell_counts_pM_out.to_csv(os.path.join(outdir,args.id + "." + args.level + "_spec_cell_log_counts_pM.tsv"), sep="\t", index=False) # save to a file)
...
def parse_args():
    ...
    parser.add_argument('--id', required=False, help='ID of snRNA dataset')
